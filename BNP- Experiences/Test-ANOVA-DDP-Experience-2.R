...
##################################################################################
###########
#           TESTE 5 REFERÊNCIA COM BNP's
###########
##################################################################################

simCode <- nimbleCode({
  for (i in 1:nsite) {
    gamma[i] ~ dnorm(inprod(mu_beta[i,1:p], x_gamma[i, 1:p]), var = tau2[i])  # random effects from mixture dist.
  ...
  for (i in 1:30) {
    mu_beta[i,1] <- muTilde_beta0[xi[i]]   
  }
  for (i in 31:60) {
    mu_beta[i,2] <- muTilde_beta1[xi[i]]   
  }
  for (i in 61:90) {
    mu_beta[i,3] <- muTilde_beta2[xi[i]]   
  }
  for (i in 91:120) {
    mu_beta[i,4] <- muTilde_beta3[xi[i]]   
  }
  ...
  for(i in 1:L) {
    ...
    muTilde_beta0[i] ~ dnorm(mu0, var = var0)
...
X_gamma <- matrix(c(c(rep(1,30), rep(0,90)),
                    c(rep(0,30), rep(1,30), rep(0,60)),
                    c(rep(0,60), rep(1,30), rep(0,30)),
                    c(rep(0,90), rep(1,30))), nrow = 120, ncol = 4)

mu_Beta <- matrix(rep(0,120*4), nrow = 120, ncol = 4)
for (i in 31:60) {
...
simModel <- nimbleModel(code = simCode,
                        ...
                        inits = list(beta0 = 1, beta = rnorm(p, 0, 1),
                                     ...
                                     muTilde_beta0 = rnorm(L, 0, 4),
                                     muTilde_beta1 = rnorm(L, 0, 0.5),
                                     muTilde_beta2 = rnorm(L, 0, 0.5),
                                     muTilde_beta3 = rnorm(L, 0, 0.5),
...
simMCMC <- buildMCMC(CsimModel, monitors = c(c("beta0", "beta02", 
                                               ...
                                               "alpha"),
                                             xi_list, w_list, tau2_list, gamma_list, prob_list, prob2_list))
...
samples5 <- runMCMC(CsimMCMC, niter = 13000, nburnin = 5000, thin = 50, summary = TRUE)


par(mfrow=c(4,4))

plot(samples5$samples[ , 'beta0'], type = 'l', xlab = 'iteration',  ylab = expression(beta0))
plot(samples5$samples[ , 'beta[1]'], type = 'l', xlab = 'iteration', ylab = expression(beta1))
plot(samples5$samples[ , 'beta[2]'], type = 'l', xlab = 'iteration', ylab = expression(beta2))
plot(samples5$samples[ , 'beta[3]'], type = 'l', xlab = 'iteration', ylab = expression(beta3))

##########

plot(samples5$samples[ , 'beta02'], type = 'l', xlab = 'iteration',  ylab = expression(beta02))
plot(samples5$samples[ , 'beta2[1]'], type = 'l', xlab = 'iteration', ylab = expression(beta21))
plot(samples5$samples[ , 'beta2[2]'], type = 'l', xlab = 'iteration', ylab = expression(beta22))
plot(samples5$samples[ , 'beta2[3]'], type = 'l', xlab = 'iteration', ylab = expression(beta23))
...
{# mu_gamma
  mu_gammaCols_23_chain1 <- samples5$samples[, c(mu_beta_list[31:60], mu_beta_list[181:210], mu_beta_list[331:360])]
  mu_gammaMn_23_chain1 <- colMeans(mu_gammaCols_23_chain1)
  hist(mu_gammaMn_23_chain1[1:30], col='red', xlim = c(-5,5), ylim = c(0,30), add = TRUE, breaks = 20, density = 5, angle = 30, border = 'red')
  hist(mu_gammaMn_23_chain1[31:60], col='orange', xlim = c(-5,5), ylim = c(0,30), add = TRUE, breaks = 20, density = 5, angle = 60, border = 'orange')
  hist(mu_gammaMn_23_chain1[61:90], col='green', xlim = c(-5,5), ylim = c(0,30), add = TRUE, breaks = 20, density = 5, angle = 80, border = 'green')
  
  mu_gammaCols_23_chain1 <- samples5$samples[, mu_beta_list[1:30]]
  mu_gammaMn_23_chain1 <- colMeans(mu_gammaCols_23_chain1)
  ...
  mu_gammaCols_23_chain1 <- samples5$samples[, mu_beta_list[31:60]]
  mu_gammaMn_23_chain1 <- colMeans(mu_gammaCols_23_chain1)
  hist(mu_gammaMn_23_chain1, xlab = 'posterior means 23', main = 'mu_gamma2', probability = FALSE, breaks = 20, col='blue', xlim = c(-6,6), ylim = c(0,30), density = 5, angle = 0, border = 'blue')
  
  mu_gammaCols_23_chain1 <- samples5$samples[, mu_beta_list[181:210]]
  mu_gammaMn_23_chain1 <- colMeans(mu_gammaCols_23_chain1)
  ...
  mu_gammaCols_23_chain1 <- samples5$samples[, mu_beta_list[331:360]]
  mu_gammaMn_23_chain1 <- colMeans(mu_gammaCols_23_chain1)
...
###############################################################################################
{{# gamma
  gammaCols_23_chain1 <- samples5$samples[, gamma_list]
  ...
  {# tau2_gamma
    tau2_gammaCols_23_chain1 <- samples5$samples[, tau2_gamma_list]
  ...
  hist(samples5$samples[, "alpha"])
}

samples5$summary['beta0',]
samples5$summary['beta[1]',]
samples5$summary['beta[2]',]
samples5$summary['beta[3]',]

samples5$summary['beta02',]
samples5$summary['beta2[1]',]
samples5$summary['beta2[2]',]
samples5$summary['beta2[3]',]
...
prob_list_estim <- samples5$samples[, prob_list]
prob_list_estim_mean <- matrix(colMeans(prob_list_estim), nrow = 120, ncol = 21)

prob2_list_estim <- samples5$samples[, prob2_list]
